...
//-----------
	...
	// $bids = array();
	// if(isset($_GET['data'])){
	// 	$fewUser = json_decode(stripslashes($_GET['data']));
	// 	$pid = $fewUser[0]->pid;
	// 	$usersId = array();
	// 	foreach($fewUser as $user){
	// 		$bids[] = projectTheme_get_bid_by_id($user->bid);
	// 	}
	// 	//var_dump($bids);
	// 	//exit;
	// }
	...
	$pid = $fewUser?$fewUser[0]->pid:$wp_query->query_vars['pid'];
...
//----------------------------------	
	...
	//}
		...
		//wp_redirect(get_permalink(get_option('ProjectTheme_my_account_page_id')));
		wp_redirect(get_permalink($pid)/*ProjectTheme_get_payments_page_url2('escrow', $pid)*/); //get_permalink(get_option('ProjectTheme_my_account_page_id')));
		exit;
	}
...
<div class="page_heading_me">
    ...
    <div class="main-pg-title">
    	<div class="mm_inn"><?php  printf(__("Choose a winner: %s",'ProjectTheme'), $post_p->post_title); ?>
                     </div>
