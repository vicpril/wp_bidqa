...
class ProjectTheme_featured_projects extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if(empty($limit) || !is_numeric($limit)) $limit = 5;
				 ...
				 ?>
                     ...
                     <?php endforeach; ?>
                     <?php else : ?> <?php $no_p = 1; ?>
                       <div class="padd100"><p class="center"><?php _e("Sorry, there are no posted projects yet."); ?></p></div>
	...
	function form($instance) { ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('show_projects_limit'); ?>"><?php _e('Show projects limit'); ?>:</label>
