...
if(isset($_POST['project_submit1']))
{
		...
		{ 
			$projectOK 							= 0;
			$MYerror['end'] 					= __('You cannot leave the Project Ending On field blank!','ProjectTheme'); 
			$class_errors['end']		= 'error_class_post_new';
		}

		if(strlen($project_title)>80) 		
		{ 
			$projectOK 							= 0;
			$MYerror['project_title'] 					= __('You enter to long the project title, maximum 80 symbols!','ProjectTheme'); 
			$class_errors['project_title']		= 'error_class_post_new';
		}
		...
		{ 
			$projectOK 							= 0;
			$MYerror['project_location_addr'] 					= __('You cannot leave the project address blank!','ProjectTheme'); 
			$class_errors['project_location_addr']		= 'error_class_post_new';
		}
		...
		if(count($_POST['project_skill_cat_multi']) == 0) 
		{ 
			$projectOK = 0; 
			$MYerror['project_skill'] 	= __('You cannot leave the project skill blank!','ProjectTheme');
			$class_errors['project_skill']		= 'error_class_post_new'; 
		}
		...
		if(isset($_POST['project_othe_skills'])){
			update_post_meta($pid, 'project_othe_skills', $_POST['project_othe_skills']);
		}
		...
		// if($_POST['payment']){
		// 	$value = intval($_POST['hourlyValue']);
		// 	update_post_meta($pid, "price", 		$value."$ per hour");
		// }else
