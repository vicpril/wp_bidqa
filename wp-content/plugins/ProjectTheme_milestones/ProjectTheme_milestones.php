...
function projectTheme_release_milestone_link($id)
{
	...
	if($perm == true)
	{
		return get_permalink($opt). "?release_id=" . $id;	
	}
	else
	{
		return get_permalink($opt). "&release_id=" . $id;	;	
...
function ProjectTheme_template_redirect_milestones()
{
	...
	if(isset($_POST['submits1yes_me_ok_p'])) {
		...
		//-------------------------		
		wp_redirect(get_permalink(get_option('ProjectTheme_my_account_milestones_id'))."?submit_ok_p=1"); exit;			
	}
...
function ProjectTheme_add_milestones_main_btn()
{
	?>
         ...
         <li> <a href="<?php echo get_permalink(get_option('ProjectTheme_my_account_milestones_id')); ?>" class="green_btn old_mm_k"><?php _e('Milestone Payments','ProjectTheme'); ?></a></li>    
