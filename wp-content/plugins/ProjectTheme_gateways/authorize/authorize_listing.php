...
function ProjectTheme_authorize_main_listing_submit_payment()
{
	...
	if(empty($business)) die('ERROR. Please input your Authorize ID.');
	//-------------------------------------------------------------------------
	...
	$cancel_url 	= get_bloginfo("siteurl").'/?p_action=authorize_listing_response&pid='.$pid;
	$response_url 	= get_bloginfo('siteurl').'/?p_action=authorize_listing_response';
...
$url            = 'https://secure.authorize.net/gateway/transact.dll'; //"https://test.authorize.net/gateway/transact.dll";
